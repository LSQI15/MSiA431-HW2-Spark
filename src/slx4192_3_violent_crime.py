...
########################################################################
# make a dataframe for only violent crimes
########################################################################
agg_crime = mydata_violent.groupBy("Beat", "Year", "Week_num", "Violent").count()
v_crime = agg_crime\
            .filter(agg_crime.Violent == 1)\
            .drop("Violent")\
...
encoder = OneHotEncoderEstimator(inputCols=["BeatIdx", "WeekNumIdx"],
                                 outputCols=["BeatVec", "WeekNumVec"]).setHandleInvalid("keep")
assembler = VectorAssembler(inputCols=["BeatVec", "Year", "WeekNumVec", "lag1",
                                       "lag2", "lag3", "lag4", "lag5", "lag6", "lag7", "lag8"], outputCol='features')
...
text_file = open("slx4192_3_violent_crime.txt", "w")
text_file.write("MSE for predicting the number of violent crime for beats at the week level is : " + str(mse) + "\n")
text_file.close()